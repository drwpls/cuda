                                      ...
                                      uint8_t *outPixels)
{
    ...
    {
        int idx = y * width + x;
        outPixels[idx] = 0.299 * inPixels[3 * idx] + 0.587 * inPixels[3 * idx + 1] + 0.114 * inPixels[3 * idx + 2];
    }
}
                     ...
                     bool useDevice = false, dim3 blockSize = dim3(1))
{
    ...
    else // use device
    {
        ...
        CHECK(cudaMalloc(&in_pixels, (size_t)(n_pixels * 3 * sizeof(uint8_t))));
		CHECK(cudaMalloc(&out_pixels, (size_t)(n_pixels * 3 * sizeof(uint8_t))));	

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(in_pixels, inPixels, n_pixels * 3 * sizeof(uint8_t), cudaMemcpyHostToDevice));
		...
		convertRgb2GrayKernel<<<gridSize, blockSize>>>(in_pixels, width, height, out_pixels); 

        CHECK(cudaPeekAtLastError());
        CHECK(cudaDeviceSynchronize());
		CHECK(cudaMemcpy(outPixels, out_pixels, n_pixels * sizeof(uint8_t), cudaMemcpyDeviceToHost));
