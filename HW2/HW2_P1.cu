...
{   
    ...
    for (int stride = numThreadPerBlock; stride > 0; stride /= 2)
    {
        if (threadIdx.x < stride)
        {
            int firstAddEle = numElemsBeforeBlk + threadIdx.x;
            int secondAddEle = firstAddEle + stride;
            if (secondAddEle < n)
                in[firstAddEle] += in[secondAddEle];
        }
