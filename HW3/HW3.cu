                               ...
                               uchar3 *outPixels)
{
    ...
    for (int i = 0; i < cell_per_thread; i++)
    {
        ...
        if (index_in_shared_mem < total_cell)
            s_inPixels[index_in_shared_mem] = inPixels[index_in_global_mem];
    }
    ...
    if (y < height && x < width)
    {
        for (int fr = 0; fr < filterWidth; fr++)
        {
            for (int fc = 0; fc < filterWidth; fc++)
            {
                ...
                int row_in_shared_mem = threadIdx.y + fr;
                int col_in_shared_mem = threadIdx.x + fc;
                uchar3 _pixel = s_inPixels[row_in_shared_mem * (filterWidth + blockDim.x - 1) + col_in_shared_mem];
                _sum.x += filter_factor * _pixel.x;
                _sum.y += filter_factor * _pixel.y;
                _sum.z += filter_factor * _pixel.z;
    ...
    if (y < height && x < width)
    {
        for (int fr = 0; fr < filterWidth; fr++)
        {
            for (int fc = 0; fc < filterWidth; fc++)
            {
                ...
                int fidx = fr * filterWidth + fc;
                float filter_factor = dc_filter[fidx];
             ...
             bool useDevice = false, dim3 blockSize = dim3(1, 1), int kernelType = 1)
{
    ...
    else // Use device
    {
        ...
        else if (kernelType == 2)
        {
            ...
            blurImgKernel2<<<gridSize, blockSize, (blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1) * sizeof(uchar3)>>>(d_inPixels, width, height, d_filter, filterWidth, d_outPixels);
        }
        else
        {
            // TODO: call blurImgKernel3
            blurImgKernel3<<<gridSize, blockSize, (blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1) * sizeof(uchar3)>>>(d_inPixels, width, height, filterWidth, d_outPixels);
        }
        timer.Stop();
